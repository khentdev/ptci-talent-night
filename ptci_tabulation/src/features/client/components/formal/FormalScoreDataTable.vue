            ...
            <table :class="TABLE_STYLES.TB" v-else>
                <thead>
                    <tr>
                        ...
                        <th :class="TABLE_STYLES.TH">Poise and Bearing (40%)</th>
                        <th :class="TABLE_STYLES.TH">Personality and Projection (30%)</th>
                        <th :class="TABLE_STYLES.TH">Appropriateness/Elegance (20%)</th>
                        <th :class="TABLE_STYLES.TH">Overall Impact (10%)</th>
                    </tr>
                </thead>
                <tbody :class="TABLE_STYLES.TBODY">
                    <tr class="hover:bg-gray-50 transition-colors" v-for="c in candidateScoreInputs"
                        ...
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c.poise_and_bearing" :max="SCORE_CRITERIA.poise_and_bearing.max"
                        ...
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c['personality/projection']"
                                :max="SCORE_CRITERIA['personality/projection'].max" min="0" :disabled="hasSubmitted"
                                @input="clampValues(c)"
                                class="w-20 px-2 py-1 disabled:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed rounded border border-gray-700 hover:border-primary text-center focus:border-primary focus:outline-none transition-colors focus:ring-primary/20 focus:ring-2" />
                        </td>
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c['appropriateness/ellegance']"
                                :max="SCORE_CRITERIA['appropriateness/ellegance'].max" :disabled="hasSubmitted" min="0"
                                @input="clampValues(c)"
                                class="w-20 px-2 py-1 disabled:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed rounded border border-gray-700 hover:border-primary text-center focus:border-primary focus:outline-none transition-colors focus:ring-primary/20 focus:ring-2" />
                        </td>
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c.overall_impact" :max="SCORE_CRITERIA.overall_impact.max"
...
<script lang="ts" setup>
    ...
    const { getMaleCandidates, getFemaleCandidates, isSubmitted } = useFormalStore()
    ...
    const SCORE_CRITERIA = {
        poise_and_bearing: { max: 40 },
        "personality/projection": { max: 30 },
        "appropriateness/ellegance": { max: 20 },
        overall_impact: { max: 10 },
    } as const;
    ...
    const clampValues = (candidate: ScoreFields) => {
        ...
        candidate.poise_and_bearing = sanitize(candidate.poise_and_bearing);
        candidate['personality/projection'] = sanitize(candidate['personality/projection']);
        candidate['appropriateness/ellegance'] = sanitize(candidate['appropriateness/ellegance']);
        candidate.overall_impact = sanitize(candidate.overall_impact);


        if (Number(candidate.poise_and_bearing) >= SCORE_CRITERIA.poise_and_bearing.max) {
            candidate.poise_and_bearing = SCORE_CRITERIA.poise_and_bearing.max.toString();
            candidate.poise_and_bearing = candidate.poise_and_bearing.replace(/\.$/, "");
        }

        if (Number(candidate['personality/projection']) >= SCORE_CRITERIA['personality/projection'].max) {
            candidate['personality/projection'] = SCORE_CRITERIA['personality/projection'].max.toString();
            candidate['personality/projection'] = candidate['personality/projection'].replace(/\.$/, "");
        }
        if (Number(candidate['appropriateness/ellegance']) >= SCORE_CRITERIA['appropriateness/ellegance'].max) {
            candidate['appropriateness/ellegance'] = SCORE_CRITERIA['appropriateness/ellegance'].max.toString();
            candidate['appropriateness/ellegance'] = candidate['appropriateness/ellegance'].replace(/\.$/, "");
        }
        if (Number(candidate.overall_impact) >= SCORE_CRITERIA.overall_impact.max) {
            candidate.overall_impact = SCORE_CRITERIA.overall_impact.max.toString();
            candidate.overall_impact = candidate.overall_impact.replace(/\.$/, "");
        }
    };
    ...
    const validateFields = () => {
        const hasError = candidateScoreInputs.value.some((c) => {
            return !c.poise_and_bearing || !c['personality/projection'] || !c['appropriateness/ellegance'] || !c.overall_impact
        });
        return hasError;
    };
