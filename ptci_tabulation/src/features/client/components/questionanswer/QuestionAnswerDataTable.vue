            ...
            <table :class="TABLE_STYLES.TB" v-else>
                <thead>
                    <tr>
                        ...
                        <th :class="TABLE_STYLES.TH">Total Score (100%))</th>
                    </tr>
                </thead>
                <tbody :class="TABLE_STYLES.TBODY">
                    <tr class="hover:bg-gray-50 transition-colors" v-for="c in candidateScoreInputs"
                        ...
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c.total_score" :max="SCORE_CRITERIA.total_score.max"
...
<script lang="ts" setup>
    ...
    const { getMaleCandidates, getFemaleCandidates, isSubmitted } = useQuestionAnswerStore()
    ...
    const clampValues = (candidate: ScoreFields) => {
        ...
        candidate.total_score = sanitize(candidate.total_score);

        if (Number(candidate.total_score) >= SCORE_CRITERIA.total_score.max) {
            candidate.total_score = SCORE_CRITERIA.total_score.max.toString();
            candidate.total_score = candidate.total_score.replace(/\.$/, "");
        }
    };
