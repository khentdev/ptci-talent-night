            ...
            <table :class="TABLE_STYLES.TB" v-else>
                <thead>
                    <tr>
                        ...
                        <th :class="TABLE_STYLES.TH">Stage Presence (40%)</th>
                        <th :class="TABLE_STYLES.TH">Figure and Fitness (30%)</th>
                        <th :class="TABLE_STYLES.TH">Poise and Bearing (20%)</th>
                ...
                <tbody :class="TABLE_STYLES.TBODY">
                    <tr class="hover:bg-gray-50 transition-colors" v-for="c in candidateScoreInputs"
                        ...
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c.figure_and_fitness"
                                :max="SCORE_CRITERIA.figure_and_fitness.max" min="0" :disabled="hasSubmitted"
                                @input="clampValues(c)"
                                class="w-20 px-2 py-1 disabled:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed rounded border border-gray-700 hover:border-primary text-center focus:border-primary focus:outline-none transition-colors focus:ring-primary/20 focus:ring-2" />
                        </td>
                        <td :class="TABLE_STYLES.TD.no_bold">
...
<script lang="ts" setup>
    ...
    const { getMaleCandidates, getFemaleCandidates, isSubmitted } = useSwimwearStore()
    ...
    type ScoreFields = {
        ...
        figure_and_fitness: string
        poise_and_bearing: string
        overall_impact: string
    }

    const SCORE_CRITERIA = {
        stage_presence: { max: 40 },
        figure_and_fitness: { max: 30 },
        poise_and_bearing: { max: 20 },
    ...
    const clampValues = (candidate: ScoreFields) => {
        ...
        candidate.figure_and_fitness = sanitize(candidate.figure_and_fitness);
        ...
        if (Number(candidate.figure_and_fitness) >= SCORE_CRITERIA.figure_and_fitness.max) {
            candidate.figure_and_fitness = SCORE_CRITERIA.figure_and_fitness.max.toString();
            candidate.figure_and_fitness = candidate.figure_and_fitness.replace(/\.$/, "");
        }
    ...
    const validateFields = () => {
        const hasError = candidateScoreInputs.value.some((c) => {
            return !c.stage_presence || !c.figure_and_fitness || !c.poise_and_bearing || !c.overall_impact
        });
        return hasError;
    };
