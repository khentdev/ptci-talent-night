            ...
            <table :class="TABLE_STYLES.TB" v-else>
                <thead>
                    <tr>
                        ...
                        <th :class="TABLE_STYLES.TH">Neatness (20%)</th>
                ...
                <tbody :class="TABLE_STYLES.TBODY">
                    <tr class="hover:bg-gray-50 transition-colors" v-for="c in candidateScoreInputs"
                        ...
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c.poise_and_bearings" :max="SCORE_CRITERIA.poise_bearing.max"
                        ...
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c.personality_and_projection"
                                :max="SCORE_CRITERIA.personality_projection.max" min="0" :disabled="hasSubmitted"
                                @input="clampValues(c)"
                                class="w-20 px-2 py-1 disabled:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed rounded border border-gray-700 hover:border-primary text-center focus:border-primary focus:outline-none transition-colors focus:ring-primary/20 focus:ring-2" />
                        </td>
                        <td :class="TABLE_STYLES.TD.no_bold">
                            <input type="text" v-model="c.neatness" :max="SCORE_CRITERIA.neatness.max"
...
<script lang="ts" setup>
    ...
    const { getMaleCandidates, getFemaleCandidates, isSubmitted } = useUniformStore()
    ...
    const SCORE_CRITERIA = {
        poise_bearing: { max: 40 },
        personality_projection: { max: 30 },
        neatness: { max: 20 },
    ...
    const clampValues = (candidate: ScoreFields) => {
        ...
        candidate.poise_and_bearings = sanitize(candidate.poise_and_bearings);
        candidate.personality_and_projection = sanitize(candidate.personality_and_projection);
        candidate.neatness = sanitize(candidate.neatness);
        candidate.overall_impact = sanitize(candidate.overall_impact);


        if (Number(candidate.poise_and_bearings) >= SCORE_CRITERIA.poise_bearing.max) {
            candidate.poise_and_bearings = SCORE_CRITERIA.poise_bearing.max.toString();
            candidate.poise_and_bearings = candidate.poise_and_bearings.replace(/\.$/, "");
        }

        if (Number(candidate.personality_and_projection) >= SCORE_CRITERIA.personality_projection.max) {
            candidate.personality_and_projection = SCORE_CRITERIA.personality_projection.max.toString();
            candidate.personality_and_projection = candidate.personality_and_projection.replace(/\.$/, "");
        }
        if (Number(candidate.neatness) >= SCORE_CRITERIA.neatness.max) {
            candidate.neatness = SCORE_CRITERIA.neatness.max.toString();
            candidate.neatness = candidate.neatness.replace(/\.$/, "");
        }
    ...
    const validateFields = () => {
        const hasError = candidateScoreInputs.value.some((c) => {
            return !c.poise_and_bearings || !c.personality_and_projection || !c.neatness || !c.overall_impact
        });
        return hasError;
    };
