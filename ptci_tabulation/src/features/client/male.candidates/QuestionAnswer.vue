<template>
    <feature-base-component :is-loading="useLoading['questionAnswerCandidates:maleInitialFetching']">
        <feature-offline-state v-if="getError['questionAnswerCandidates:maleFetchOffline'] || offline" />
        <data-loading-state v-else-if="useLoading['questionAnswerCandidates:maleInitialFetching']" />
        <template v-else>
            <feature-header :popup-fn="openSevenRules" :has-icon="true" title="Male Candidate Question and Answer Scores"
                action-fn-name="Submit" action-fn-title="Submit question and Answer scores for candidates"
                popup-fn-name="View Top 7 Rules" popup-fn-title="View top 7 rules" :action-fn="openConfirmationModal"
                description="Provide question and answer performance scores for each male contestant." />

            <question-answer-data-table input-key-submitted="male-questionAnswer-submitted" candidate-type="male"
                :retry-fn="refetchMaleQuestionAnswerFeat" ref="questionAnswerScoreDataTable"
                input-key="male-questionAnswer-scores"
                :is-loading="useLoading['questionAnswerCandidates:maleFetchRefresh']"
                :is-error="getError['questionAnswerCandidates:maleFetchServerError']" />
        </template>
    ...
    <confirmation-modal title="Submit Question and Answer Scores for Candidates"
        :is-loading="useLoading['questionAnswerCandidates:createMaleQuestionAnswerScore']"
...
<script lang="ts" setup>
    ...
    const { refetchMaleQuestionAnswerFeat, createQuestionAnswerScoreMale, enableMale } = useQuestionAnswerStore()
    ...
    const openConfirmationModal = () => {
        if (
            getError["questionAnswerCandidates:maleFetchOffline"] ||
            getError["questionAnswerCandidates:maleFetchServerError"] ||
            offline.value
        )
            return;
    ...
    const handleCreateQuestionAnswerScore = async () => {
        ...
        await createQuestionAnswerScoreMale(payload);
    }
</script>
