<template>
    <feature-base-component :is-loading="useLoading['uniformCandidates:maleInitialFetching']">
        <feature-offline-state v-if="getError['uniformCandidates:maleFetchOffline'] || offline" />
        <data-loading-state v-else-if="useLoading['uniformCandidates:maleInitialFetching']" />
        <template v-else>
            <feature-header :popup-fn="openSevenRules" :has-icon="true" title="Male Candidate Uniform Scores"
                ...
                description="Provide uniform performance scores for each male contestant." />

            <uniform-score-data-table input-key-submitted="male-uniform-submitted" candidate-type="male"
                :retry-fn="refetchMaleUniformUniformFeat" ref="uniformScoreDataTable" input-key="male-uniform-scores"
                :is-loading="useLoading['uniformCandidates:maleFetchRefresh']"
                :is-error="getError['uniformCandidates:maleFetchServerError']" />
        </template>
    ...
    <confirmation-modal title="Submit Uniform Scores for Candidates"
        :is-loading="useLoading['uniformCandidates:createMaleUniformScore']"
...
<script lang="ts" setup>
    ...
    const { refetchMaleUniformUniformFeat, createUniformScoreMale, enableMale } = useUniformStore()
    ...
    const openConfirmationModal = () => {
        if (
            getError["uniformCandidates:maleFetchOffline"] ||
            getError["uniformCandidates:maleFetchServerError"] ||
            offline.value
        )
            return;
    ...
    const handleCreateUniformScore = async () => {
        ...
        await createUniformScoreMale(payload);
    }
</script>
